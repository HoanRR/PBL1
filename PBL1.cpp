...
vector<string> Dulieu_Dangnhap(){
    fstream fi("data\\user.txt", ios::in | ios::out | ios::app);
    string line;
...
void print_page(Node *start, Node *end, int widths[],int &cnt) {
    Node *cur = start;
    while (cur != end) {
        cout << char(186);
        cout << cnt;
        ...
        cout << char(186);
        cout << cur->sach->namsx;
        for (int j = to_string(cur->sach->namsx).length(); j < widths[5]; j++) cout << ' ';
        cout << char(186);
        cout << cur->sach->soluong;
        for (int j = to_string(cur->sach->soluong).length(); j < widths[6]; j++) cout << ' ';
        cout << char(186);
        string tt = cur->sach->Trang_thai ? "Con" : "Het";
        cout << tt;
        for (int j = tt.length(); j < widths[7]; j++) cout << ' ';
        cout << char(186) << "\n";
        cur = cur->next;
        cnt++;
    }
...
void print_lib(Node* head) {
    ...
    while (cur != NULL) {
        ...
        print_page(start, cur, widths,cnt);
...
void Add_RecycleBin(Node *rac, Book *b) {
    if (b == nullptr) return; 
    pushend(rac, b);
    ofstream fo("data\\thung_rac.txt", ios::app);
    if (!fo.is_open()) {
...
void KhoiPhuc_TatCa(Node *head,Node *rac){
    ...
    save_to_file(head,"data\\thuvien.txt");
    rac->next = NULL;
    save_to_file(rac,"data\\thung_rac.txt");
}
...
void KhoiPhuc(Node* head, Node* rac) {
    ...
    while (check) {
        system("cls");
        int x = 70, y = 2, width = 80, height = 14;
        drawBox(x, y, width, height);
...
void xoa_sach(Node *head, Node *rac) {
    ...
    while(check){
    system("cls");
    int x = 70, y = 2, width = 80, height = 22;
    drawBox(x, y, width, height);
...
void trang_chu_admin(Node *head) {
    Node *rac = Head();
    read_file(rac, "data\\thung_rac.txt");
    bool cnt = true;
    while (cnt) {
        system("cls");
        int x = 70, y = 3, width = 70, height = 24;
        drawBox(x, y, width, height);
...
void print_user(const string &ten) {
    system("cls");
    int x = 70, y = 2, width = 80, height = 25;
    drawBox(x, y, width, height);
    gotoXY(x + (width - 20) / 2, y + 1); cout << "DANH SACH SACH DA MUON";

    string temp = "data\\user\\" + ten + ".txt";
    ifstream fi(temp);
    string line;
    int i = 1;
    int k=1;
    ...
    while (getline(fi, line)) {
        ...
        gotoXY(x + 2, y -3 + k * 6); cout << "Sach: " << i;
        gotoXY(x + 2, y  -2 + k * 6); cout << "ID: " << ID;
        gotoXY(x + 2, y - 1 + k * 6); cout << "Ten: " << ten;
        gotoXY(x + 2, y  + k * 6); cout << "Tac Gia: " << tacgia;
        gotoXY(x + 2, y +1 + k * 6); cout << "Thoi gian muon: " << ThoiGian;
        i++;
        k++;
        if (i % 3 == 1){
            gotoXY(x+2,y + height -4);
            cout <<"Nhan phim -> de xem trang sau hoac ESC de thoat";
            char ch = _getch();
            if (ch == 27) break;
            else {
                for(int j=1 ; j<=3;j++){
                    clearLine(x+2,y-3+j*6,width-4);
                    clearLine(x+2,y-2+j*6,width-4);
                    clearLine(x+2,y-1+j*6,width-4);
                    clearLine(x+2,y+j*6,width-4);
                    clearLine(x+2,y+1+j*6,width-4);
                }
                clearLine(x+2,y+height-4,width-4);
                k = 1;
            }
...
void trang_chu_user(const string& TenDangNhap, Node *head) {
    ...
    while (cnt) {
        system("cls");
        int x = 70, y = 3, width = 70, height = 18;
        drawBox(x, y, width, height);
...
string dangnhap() {
    ...
    for (int i = 3; i > 0; i--) {
        system("cls");
        int x = 70, y = 5, width = 60, height = 15;
        drawBox(x, y, width, height);
...
int main() {
    Node* head = Head();
    read_file(head,"data\\thuvien.txt"); // đọc vào thư 
    showIntro();
    system("cls");
    menuLoop(head);
    deleteList (head);
